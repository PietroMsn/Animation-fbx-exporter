...
## fa l override di vertscore contenuta in lbs.py e al suo interno la chiama
def verts_core(pose, v, J, weights, kintree_table, bs_style, want_Jtr=False, xp=chumpy):
    
    ####  hasattr ritorna true se nel primo argomento ce un attributo chiamato come il secondo argomento
    # assert serve per lanciare in test una condizione e catturarne gli errori

    #controlla che ci siano gli attributi dterms in pose, v, J weights (dovrebbero essere gli oggetti chumpy.ch.Ch)
    if xp == chumpy:
    ...
    assert(bs_style=='lbs')
    #chiama verts_core in lbs.py (stessi argomenti tranne bs_Style)
