...
## esegue la trasformazione rigida
def global_rigid_transformation(pose, J, kintree_table, xp):
    ...
    ## esegue la formula di Rodrigues su lambda x
    if xp == chumpy:
...
## crea le dipendenze dei vertici con i giunti in base ai pesi
def verts_core(pose, v, J, weights, kintree_table, want_Jtr=False, xp=chumpy):
    ...
    # fa la dot multiplication tra A e weights.T
    T = A.dot(weights.T)

    # calcolo la shape a riposo
