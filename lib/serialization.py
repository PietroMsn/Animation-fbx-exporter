...
def ready_arguments(fname_or_dict):
    ...
    if want_shapemodel:
        #calcolo di Bs = sum beta*S (orthonormal principal components of shap displacements) dimensione: 3*numVertici x numBeta (3x6890x10)
        dd['v_shaped'] = dd['shapedirs'].dot(dd['betas'])+dd['v_template']   
        v_shaped = dd['v_shaped']
        #calcolo posizione dei giunti tramite la regressor matrix (che trasforma i rest vert in rest joints)
        J_tmpx = MatVecMult(dd['J_regressor'], v_shaped[:,0])        
        J_tmpy = MatVecMult(dd['J_regressor'], v_shaped[:,1])        
        J_tmpz = MatVecMult(dd['J_regressor'], v_shaped[:,2])        
        dd['J'] = ch.vstack((J_tmpx, J_tmpy, J_tmpz)).T 
        #calcolo della Tp = T + Bs + Bp  (posedirs o P matrix of all 207 pose blend shapes (3N x 9K con k num joint e N num vert))   
...
def load_model(fname_or_dict):
    ...
    ### Jtr e di tipo chumpy.reordering.Concatenate, result e chumpy.reordering.Select


    ## chiamo verts_core (in verts) con gli argomenti estratti sopra da dd (ready_arguments estrae gli attributi e li ritorna in dd). 
    ## 
